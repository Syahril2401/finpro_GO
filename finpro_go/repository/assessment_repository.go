...
type AssessmentRepository interface {
	...
	GetLastSummary(userID uint) (*model.ResultSummary, error)
...
func (r *assessmentRepository) GetLastSummary(userID uint) (*model.ResultSummary, error) {
	var summary model.ResultSummary
	err := r.db.Where("user_id = ?", userID).Order("id desc").First(&summary).Error
	return &summary, err
}
